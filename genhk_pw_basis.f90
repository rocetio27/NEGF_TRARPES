subroutine genhk_pw_basis(ns,pp_vec,gpz_vecs,h_k_tb,h_k_pw)
use modconstants
use modmain
use modpulse
implicit none
...
enddo
enddo
 h_k_pw(igpz,jgpz)=h_k_pw(igpz,jgpz)*phi_p(pp_vec+gpz_vec_i)*conjg(phi_p(pp_vec+gpz_vec_j))/cell_area/h_box
enddo
enddo
endif !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
do jgpz=1,ns
  ...
  if(igpz.eq.jgpz) then
    kinetic_mat_el=sum((pp_vec+gpz_vec_i)**2)/2d0
  endif

  q_vec=gpz_vec_i-gpz_vec_j
  do iatom=1,natom
    phase=-(q_vec(1)*atom_pos(iatom,1)+q_vec(2)*atom_pos(iatom,2)+q_vec(3)*atom_pos(iatom,3))
    pot_mat_el=pot_mat_el-4d0*pi*atomic_nums(iatom)/(norm2(q_vec)**2+mu(iatom)**2)*(cos(phase)+ci*sin(phase))
  enddo

  h_k_pw(igpz,jgpz)=kinetic_mat_el+pot_mat_el/cell_area/h_box
enddo
enddo
endif
