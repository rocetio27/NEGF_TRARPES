subroutine set_pulse_variables(itmax1,t_pump_start1,t_pump_end1,t_probe_start1,&
                               t_probe_end1,it_observe1,t_error1,exitmax1)
use modpulse
implicit none
integer, intent(out) :: itmax1
real(8), intent(out) :: t_pump_start1,t_pump_end1,t_probe_start1,t_probe_end1
integer, intent(out) :: it_observe1
real(8), intent(out) :: t_error1
integer, intent(out) :: exitmax1
if((-hd_pump).le.(delta_t-hd_probe)) then
  tshift=-hd_pump !coordinate shift to fit the left-side first
else
...
t_observe=(itmax1-1)*dt
t_pump_start1=-tshift-hd_pump
t_pump_end1=-tshift+hd_pump
t_probe_start1=(t_pump_start1+t_pump_end1)/2d0+delta_t-hd_probe
t_probe_end1=(t_pump_start1+t_pump_end1)/2d0+delta_t+hd_probe
write(*,*) "time when the probe ends:",t_probe_end1
!it_observe1=floor(t_observe/dt)
it_observe1=floor(t_probe_end1/dt)
t_error1=ceiling(t_pump_start1/dt)*dt-t_pump_start1
exitmax1=2*itmax1-1
if(t_probe_start1.gt.t_observe) then
  write(*,*) "observing time",t_observe, "is ealier than the probe-start time", t_probe_start1
  stop
endif
